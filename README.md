...
A notificação é entregue pelo sistema operacional mesmo quando a aba do radar não está em primeiro plano. No Android, o clique usa uma Intent explícita para o pacote oficial do LinkedIn e mantém a URL web como fallback. No iPhone/iPad, a notificação abre diretamente a URL HTTPS da vaga como Universal Link, permitindo ao iOS encaminhá-la ao aplicativo do LinkedIn quando ele está instalado e associado; se isso não estiver disponível, o navegador assume automaticamente. No desktop, o link continua sendo aberto normalmente no LinkedIn. O estado do botão é restaurado após recarregar a página e cada navegador/dispositivo mantém sua própria assinatura, portanto ativar ou desativar em uma máquina não altera as demais. Assinaturas expiradas são removidas automaticamente do banco.
